...
def main():
    ...
    Main_Data = pd.concat([Main_Train_Data, Main_Test_Data, Main_Validation_Data])
    ...
    for dset_type, dset in zip(DSET_TYPES, DSET):
        ...
        for index, row in dset.iterrows():
            ...
            path = path.split('/')[-1]
            if label == 0:
                image.save('./wbc_reorg/' + dset_type + 'EOSINOPHIL/' + path)
            elif label == 1:
                image.save('./wbc_reorg/' + dset_type + 'LYMPHOCYTE/' + path)
            elif label == 2:
                image.save('./wbc_reorg/' + dset_type + 'MONOCYTE/' + path)
            else:
                image.save('./wbc_reorg/' + dset_type + 'NEUTROPHIL/' + path)
